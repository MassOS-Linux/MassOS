...
For (c), you can find the patch that MassOS applies to GRUB in the `patches/` directory of the MassOS source tree (named `grub-2.12-uefisecureboot-installsigned.patch`). For (a) and (b), the `grub-mkstandalone` utility does this job for us. It will generate a fully standalone GRUB image, containing all modules, and containing an embedded stub configuration file. As part of the GRUB package in the MassOS build system, we generate 3 different standalone images, each with a specific purpose:
...
Although the GRUB standalone images bundle every GRUB module within themselves, this is only done for the purpose of secure boot support (since, as previously mentioned, GRUB won't allow loading modules from the disk in secure boot mode). Not all of them are used by the bootup process itself.

Some common disk and filesystem modules are configured (via use of the `--modules=` argument to `grub-mkstandalone`) to be automatically pre-loaded by the GRUB image on bootup. This is only done as it is required by some media creation software (e.g., Rufus) to be able to properly detect the filesystems "supported" by the GRUB image. All other needed modules are loaded by the embedded stub configuration file (which is the preferred method).
